...
for row in file1:
    ...
    if row[target_idx] == '':  #If target is blank, skip row
        ...
        target.append(float(
            row[target_idx]))  #If pre-binned class, change float to int
...
if lv_filter == 1:
    ...
    for field in temp:
        header.append(field)
    data_np = np.delete(data_np, temp_del,
                        axis=1)  #Deletes non-selected features by index
...
data_train, data_test, target_train, target_test = train_test_split(
    data_np, target_np, test_size=0.35)
...
if cross_val == 0:
    ...
    scores_AUC = metrics.roc_auc_score(target_test,
                                       clf.predict_proba(data_test)[:, 1])
    print('Random Forest AUC:',
          scores_AUC)  #AUC only works with binary classes, not multiclass
...
if cross_val == 1:
    ...
    scores_AUC = scores[
        'test_roc_auc']  #Only works with binary classes, not multiclass
    print("Random Forest AUC: %0.2f (+/- %0.2f)" %
          (scores_AUC.mean(), scores_AUC.std() * 2))
